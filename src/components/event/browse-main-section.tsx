        ...
        <div className="mx-auto grid max-w-[1366px] gap-10 lg:grid-cols-[180px_minmax(0,1fr)]">
          <aside className="text-[#3A3A3A]">
            ...
            <div className="border-t border-[#E5E1DA] pt-5">
              <h3 className="text-[11px] font-bold tracking-[0.12em] text-[#6F6F6F]">MAP</h3>
              <div className="mt-3 overflow-hidden rounded-[14px] border border-[#E0DDD6] bg-[#F6F3ED]">
                <div className="relative h-[120px] bg-[linear-gradient(180deg,rgba(176,138,69,0.06),rgba(176,138,69,0.02)),linear-gradient(90deg,rgba(0,0,0,0.03)_1px,transparent_1px),linear-gradient(rgba(0,0,0,0.03)_1px,transparent_1px)] bg-[size:auto,58px_58px,58px_58px]">
                  <span className="absolute left-[42px] top-[30px] h-3.5 w-3.5 rounded-full bg-[var(--ae-accent)] ring-4 ring-[#DCC89E]/50" />
                  <span className="absolute left-[96px] top-[66px] h-3.5 w-3.5 rounded-full bg-[var(--ae-accent)] ring-4 ring-[#DCC89E]/50" />
                  <button
                    type="button"
          ...
          <div>
            <div className="flex flex-col gap-5 border-b border-[#E5E1DA] pb-5 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-[14px] text-[#585858]">
                <span className="font-semibold text-[#2A2A2A]">24 events</span> matching your filters
              </p>
              <div className="flex items-center gap-3 self-start sm:self-auto">
                <button
                  type="button"
                  className="inline-flex py-2 items-center gap-2 rounded-[10px] border border-[#E0DDD6] bg-white px-4 text-[12px] font-medium text-[#2E2E2E]"
                >
                  Sort: Date (soonest)
                  <ChevronRight className="h-3.5 w-3.5 rotate-90 text-[#6F6F6F]" strokeWidth={1.8} />
                </button>
                <div className="inline-flex overflow-hidden rounded-[10px] border border-[#DAD7D0]">
                  <button
                    type="button"
                    className="inline-flex py-2 justify-center items-center gap-1.5 bg-[#1F1F1F] px-4 text-[12px] font-semibold text-white"
                  >
                  ...
                  <button
                    type="button"
                    className="inline-flex  py-2 justify-center items-center gap-1.5 bg-white px-4 text-[12px] font-medium text-[#6B6B6B]"
                  >
                    Calendar
                  </button>
                </div>
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {browseEvents.map((event) => (
                ...
                >
                  ...
                  >
                    <div className="absolute left-3 top-3 rounded-[10px] bg-[#242424] px-3 py-2 text-white shadow-sm">
                      <p className="text-[9px] font-semibold uppercase leading-none tracking-[0.1em]">
                        {event.month}
                      </p>
                      <p className="mt-1 text-[20px] font-semibold leading-none">
                        {event.day}
                      </p>
                      <p className="mt-1 text-[8px] font-medium leading-none text-white/80">2026</p>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="px-4 pb-4 pt-4">
                    ...
                    <h3 className="mt-3 max-w-[16ch] text-[15px] font-semibold leading-[1.22] text-[#252525]">
                      {event.title}
                    </h3>
                    <p className="mt-3 max-w-[28ch] text-[11px] leading-[1.65] text-[#7C7C7C]">
                      {event.summary}
                    </p>

                    <div className="mt-4 border-t border-[#ECE7E0] pt-3">
                      <div className="flex items-center justify-between gap-4">
                        <div className="inline-flex items-center gap-1.5 text-[11px] text-[#7B7B7B]">
                          <MapPin className="h-3.5 w-3.5" strokeWidth={1.8} />
                          {event.location}
                        </div>
                        <span className="text-[12px] font-medium text-[var(--ae-accent)]">
                          {event.price}
                        </span>
                      </div>
                    </div>
