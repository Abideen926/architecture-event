export function SubmitEventHeroSection() {
  return (
    <section className="bg-white pt-[68px]">
      <div className="ae-container">
        <div className="mx-auto max-w-[1040px] text-center">
          <p className="ae-section-kicker">SUBMIT AN EVENT</p>
          <h1 className="ae-section-heading mt-5 text-[46px] leading-[0.98] md:text-[58px]">
            List your event where the industry is looking
          </h1>
          <p className="ae-section-description mx-auto mt-5 max-w-[620px] text-[18px] leading-[1.7]">
            Choose a listing option to begin. Once you select a package we&apos;ll open
            the submission form for you.
          </p>
        </div>
      </div>
    </section>
  );
}
