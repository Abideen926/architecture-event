...
export function AdminOrganizersPage() {
  ...
  async function handleToggleActive(
    ...
    try {
      ...
      toast.success(
        isActive ? "Organizer deactivated" : "Organizer reactivated",
      );
    } catch (error) {
      toast.error("Couldn't update organizer", {
            ...
            >
              ...
              <div className="flex justify-end gap-4 text-[13px]">
                <button
                  type="button"
                  onClick={() =>
                    handleToggleActive(user.id, user.isActive, user.fullName)
                  }
                  className="text-[#4E4E4E] transition-colors hover:text-foreground"
                >
