...
export function AdminReportsPage() {
  ...
  const maxVolume = Math.max(
    ...eventVolume.map((entry) => Math.max(entry.submitted, entry.published)),
    1,
  );
    ...
    ? [
        {
          ...
          percentage:
            leadVolume.total > 0
              ? (leadVolume.saved.total / leadVolume.total) * 100
              : 0,
          tone: "dark" as const,
        },
        {
          ...
          percentage:
            leadVolume.total > 0
              ? (leadVolume.registered.total / leadVolume.total) * 100
              : 0,
